...
const ChatBox = ({ currentUser, receiverId, receiverName }: any) => {
  ...
  // Safe history loading without deleting live messages
  useEffect(() => {
    if (isOpen && receiverId && myId) {
      API.get(`/chat/history/${receiverId}`).then(({ data }) => {
        setMessages((prev) => {
          // Merge history with any existing local/optimistic messages to prevent data loss
          const existingIds = new Set(data.map((m: any) => m._id));
          const uniqueLocal = prev.filter((m) => !existingIds.has(m._id));
