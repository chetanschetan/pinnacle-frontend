...
// --- FILE CARD COMPONENT (FIXED FOR S3 & LOCAL FALLBACK) ---
const FileCard = ({ fileObj, onDelete }: { fileObj: any, onDelete: (p: string) => void }) => {
  const path = typeof fileObj === 'string' ? fileObj : fileObj?.path;
  ...
  // ✅ S3 URLs start with 'http', use them directly. Local paths fall back to Render.
  const url = path.startsWith('http') 
    ? path 
    : `https://pinnacle-backend-1-qyyx.onrender.com/${path.replace(/\\/g, '/')}`;
