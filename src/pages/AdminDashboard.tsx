    ...
    const fetchAndGroup = async () => {
      try {
        ...
        const groups = data.reduce((acc: any, req: any) => {
          ...
          // Agar user role admin hai, toh hume client ki request chahiye, isliye try karenge ki agar document mein koi client name/email ya alag ID ho toh wo lein, warna admin ko skip karein
          const isUserAdmin = userObj.role === 'admin';
          
          // Client ID nikalne ka tareeqa (agar user admin hai toh req ke andar client details dhoondo)
          const userId = isUserAdmin 
            ? (req.client?._id || req.clientId || req._id) 
            : (userObj._id || userObj.id);

          if (!userId) return acc;

          if (!acc[userId]) {
            // Agar user admin return ho raha hai data mein, toh fallback client name/email use karo jo request ke sath aaya ho
            let finalUserInfo = userObj;
            if (isUserAdmin) {
              finalUserInfo = {
                _id: userId,
                fullName: req.clientName || req.name || 'Valued Client',
                email: req.clientEmail || req.email || '',
                role: 'client'
              };
            }
        ...
        {loading ? (
          ...
          <>
            ...
            {filteredUsers.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredUsers.map((client) => {
                  const clientName = client.userInfo?.fullName || client.userInfo?.name || 'Valued Client';
                  const clientEmail = client.userInfo?.email || 'No email provided';
                  const formattedName = clientName.trim().replace(/\s+/g, '_');
